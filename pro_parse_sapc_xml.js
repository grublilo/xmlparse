...
const parseXML=require('./lib/parse-sapc-xml.js')
...
function dealfiles(files) {
    files.forEach(sfile => {
        ...
        ws.on('finish',()=>{
            ...
            const parsed_file=sfile.replace(/xml\.gz/g,'parsed')
            const epgxml=parseXML(fs.readFileSync(dfile))
            epgxml.filename=path.basename(sfile)
